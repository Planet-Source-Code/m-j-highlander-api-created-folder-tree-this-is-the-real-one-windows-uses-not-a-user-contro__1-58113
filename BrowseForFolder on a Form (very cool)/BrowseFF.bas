...
Public gs_CurrentDirectory As String
Private DialogWindow As Long
Private SysTreeWindow As Long
Private CancelbuttonWindow As Long
Public DialogContainer As Object

'Standard BrowseForFolder dialog
Public Sub BrowseForFolder(StartDir As String, Optional IncludeFiles As Boolean = False)
    Dim lpIDList As Long
    Dim szTitle As String
    Dim sBuffer As String
    Dim tBrowseInfo As BrowseInfo
    gs_CurrentDirectory = StartDir & vbNullChar
    With tBrowseInfo
        .hwndOwner = GetDesktopWindow
        .lpszTitle = lstrcat(szTitle, "")
        .ulFlags = BIF_RETURNONLYFSDIRS + BIF_STATUSTEXT
        If IncludeFiles Then .ulFlags = .ulFlags + BIF_BROWSEINCLUDEFILES
...
Select Case uMsg
    Case BFFM_INITIALIZED
        ...
        Call SendMessage(hwnd, BFFM_SETSELECTION, 1, gs_CurrentDirectory)
    ...
    Case BFFM_SELCHANGED
        ...
        ' BFFM_SELCHANGED happens *befor* BFFM_INITIALIZED, returning a null path, so we better make sure
        If Ret > 0 Then gs_CurrentDirectory = sBuffer
        Form1.PathChange
End Select
...
Public Sub ChangePath(mPath As String)
    ...
    gs_CurrentDirectory = mPath 'update variable
    'Tell BrowseForFolder what to do
    Call SendMessage(DialogWindow, BFFM_SETSELECTION, 1, gs_CurrentDirectory)
End Sub
